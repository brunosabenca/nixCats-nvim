{
  description = "My nixCats flake based on an example";
        ...
        nixCats =
          ...
          }@misc:
          {
            ...
            settings = {
              ...
              # see :help nixCats.flake.outputs.settings for all of the settings available
              wrapRc = true;
        ...
        regularCats =
          { pkgs, ... }@misc:
          {
            settings = {
              suffix-path = true;
              suffix-LD = true;
              # IMPURE: loads config from vim.fn.stdpath('config') for live editing without rebuild
              wrapRc = false;
              configDirName = "nixCats-nvim";
              aliases = [ "testCat" ];
              neovim-unwrapped = inputs.neovim-nightly-overlay.packages.${pkgs.stdenv.hostPlatform.system}.neovim;
